...
parser.add_argument(
    "display_width", type=int, help="an integer representing the display width"
)
parser.add_argument(
    "display_height", type=int, help="an integer representing the display height"
)
...
parser.add_argument(
    ...
    default=False,
    required=False,
    help="Specify whether the CSV data is relative or absolute",
)
    ...
    if len(raw[0]) == 2:
        if args.relative:
            gaze_data = list(
                map(
                    lambda q: (
                        int(display_width * float(q[0])),
                        int(display_height * float(q[1])),
                        1,
                    ),
                    raw,
                )
