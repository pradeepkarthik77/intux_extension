...
def generate_clickmap_image(roll_number):
    # Connect to MongoDB
    ...
    plt.savefig(f'{roll_number}.png', bbox_inches='tight', pad_inches=0)

    # Close the plot to release resources
    plt.close()

if __name__ == "__main__":
    # Check if a command-line argument (roll number) is provided
    if len(sys.argv) != 2:
        print("Usage: python script.py <roll_number>")
        sys.exit(1)

    # Get roll number from command-line arguments
    roll_number = sys.argv[1]

    generate_clickmap_image(roll_number)
    # Upload the image to Google Cloud Storage
    bucket_name = 'intux_clickmap'  # Replace with your bucket name
    key_json_path = "../finalyear-409412-5412169777f2.json"  # Replace with your service account key JSON path
    remote_file_path = f'{roll_number}.png'  # Remote file path in your bucket
    upload_to_gcs(bucket_name, f"{roll_number}.png", remote_file_path, key_json_path)